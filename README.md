...
-------
        ...
        Step 5: How to manage data within your Docker containers.
           Once the ProkSeq pipeline is working with the example files, one can go for the real data.
           ...
           Considering the user has all the required data files (sample[fq/fastq], GTF, BED, genome/transcript files, etc) in /home/user/prokseqData, the above command will mount the directory /home/user/prokseqData in /root/prokseq/. User can run the pipeline with appropriate sample files and parameter files inside the docker. Once done, the user can exit the docker. The output will remain in the /home/user/prokseqData.
